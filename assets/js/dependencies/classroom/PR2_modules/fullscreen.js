...
PR2.fullscreen = (function(self) {
	...
	if (document.cancelFullscreen) {
		window.addEventListener('fullscreenchange', function() {
			_fullscreen_handler(document.fullScreenElement);
		});
		...
		window.addEventListener('webkitfullscreenchange', function() {
			_fullscreen_handler(document.webkitFullscreenElement);
		});
	...
	return {
         ...
         *
		 * @param {String} [id] the id of the element, if no id is given the body element requests fullscreen access
		 */
		 ...
		 * @param {HTMLElement} The element that has fullscreen (null if not in fullscreen mode)
		 */
